...
build:
	GOPRIVATE="github.com/mfa_kpi/*" go build -o build/_output/onos-kpimon ./cmd/onos-kpimon
...
test: build deps linters license
	go test -race github.com/mateofloreza/mfa_kpi/pkg/...
	go test -race github.com/mateofloreza/mfa_kpi/cmd/...

jenkins-test:  # @HELP run the unit tests and source code validation producing a junit style report for Jenkins
jenkins-test: deps license linters
	TEST_PACKAGES=github.com/mateofloreza/mfa_kpi/... ./build/build-tools/build/jenkins/make-unit

buflint: #@HELP run the "buf check lint" command on the proto files in 'api'
	docker run -it -v `pwd`:/go/src/github.com/mateofloreza/mfa_kpi \
		-w /go/src/github.com/mateofloreza/mfa_kpi/api \
		bufbuild/buf:${BUF_VERSION} check lint

protos: # @HELP compile the protobuf files (using protoc-go Docker)
protos:
	docker run -it -v `pwd`:/go/src/github.com/mateofloreza/mfa_kpi \
		-w /go/src/github.com/mateofloreza/mfa_kpi \
...
onos-kpimon-docker:
	...
	docker build . -f build/onos-kpimon/Dockerfile \
		-t mateofloreza/mfa_kpi:${ONOS_KPIMON_VERSION}
	@rm -rf vendor
...
kind: images
	...
	kind load docker-image mateofloreza/mfa_kpi:${ONOS_KPIMON_VERSION}

all: build images

publish: # @HELP publish version on github and dockerhub
	./build/build-tools/publish-version ${VERSION} mateofloreza/mfa_kpi
...
clean:: # @HELP remove all the build artifacts
	...
	go clean -testcache github.com/mateofloreza/mfa_kpi/...
